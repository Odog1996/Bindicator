...
BinDayInfo HTMLParser::getBinDay(std::string bin)
{
    ...
    if(binColourlocation != std::string::npos)
    {
        ...
        std::string dateStr = parseInfo(binColourlocation, dateString, dataEnd);
        size_t pos = dateStr.find(" ");
        int day = std::stoi(dateStr.substr(0, pos));
        int month = convertMonth(dateStr.substr(pos + 1, (dateStr.size() - (pos + 1))));
